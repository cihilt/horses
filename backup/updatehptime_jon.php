...
$sql_formulas = "SELECT `secpoint`,`timer` FROM `tbl_formulas` WHERE id=1";
...
$race_id_var_get = isset($_GET['race_id']) && $_GET['race_id'] ? $_GET['race_id'] : 0;
...
exit();
	...
	if($action == 'sectional') {
		...
		echo 'Handicap Resetted for '.$limit.'. <a href="./updatehptime.php">Click Here</a> to go back';
	}
    ...
    else if($action == 'ranksectionalavgrating') {
        // rank
        if ($race_id_var_get) {
            $sql2 = "SELECT `race_id` FROM `tbl_races` WHERE `race_id`='$race_id_var_get' ORDER by `race_id` ASC";
        } else {
