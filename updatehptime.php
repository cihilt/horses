...
$race_id_var_get = 0;
if (isset($_GET['race_id']) && $_GET['race_id']) {
    ...
    <center>
        ...
        if ($result_races->num_rows > 0) {
            while ($result_race = $result_races->fetch_object()) {
                $raceIdOptions .= '<option value="'.$result_race->race_id.'"'.($race_id_var_int
        ...
        <form method="post" id="race_form">
            <select name="race_id" onchange="document.getElementById('race_form').submit();">
                <option value="">Select race_id</option>
                <?= $raceIdOptions ?>
            </select>
            <a href="" style="color: #000">Select RaceID</a></br>
        </form>

        <h3>Default Algorithm</h3>
        <?php require_once 'includes/default_algorithm.php'; ?>

        <h3>Remove meetings by date</h3>
        <?php require_once 'includes/remove_meeting.php'; ?>

        <h3>Remove selected race</h3>
        <?php require_once 'includes/remove_race.php'; ?>
        ...
        <a href="./updatehptime.php?limit=50000&do=datarating<?= $race_id_var ?>">Rating</a> |
        <a href="./updatehptime.php?limit=50000&do=distance_rank<?= $race_id_var ?>">Distance Rank</a><br />
        
        <h4>Setup the Rankings in one go</h4>
        <a href="./updatehptime.php?limit=50000&do=ranksectionalavgrating<?= $race_id_var ?>">Rank + Sectional Average + Rating</a>
    </center>
    <?php
} else {
    $action = $_GET['do'];
